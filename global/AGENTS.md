...
- `scope` 可选
- `summary` 使用英文、动词开头、长度 ≤ 50 字、不加句号
...
- 默认文档根目录：`~/Dev/Codex`。
- 默认按“当前仓库所在文件夹层级”在该根目录下建立子目录后再落文档；例如仓库 `~/Dev/workspace/stockstock\trading_system` 的默认文档目录为 `~/Dev/Codex/stock\trading_system`。
- 若当前仓库路径中包含 `Dev/`，默认取其后的相对层级作为子目录；若不包含，则回退为“仓库父目录名/仓库目录名”。若用户明确指定其他路径，以用户要求为准。
...
- 当前会话内并行派发遵循 `dispatching-parallel-agents`：明确每个子代理的 `scope_read` / `scope_write` / 验证方式与交付契约，避免共享文件写冲突。
...
- 分支收尾遵循 `finishing-a-development-branch`：在 merge / PR / 保留 / 丢弃之间显式决策，确认验证通过后再清理 worktree。
...
- 本文件默认采用以下主干整合方式：
  - 入口与说明：`using-superpowers`
  - 实现前：`brainstorming -> writing-plans`
  - 计划执行：`executing-plans`（顺序批量）/ `subagent-driven-development`（并行子代理）
  - 并行派发：`dispatching-parallel-agents`
  - worktree 协作：`using-git-worktrees`
